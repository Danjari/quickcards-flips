...
export const createFlashcards = async (jsonElements) => {
  const openai = new OpenAI({
    apiKey: process.env.OPENAI_API_KEY,
  });
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      { role: "system", content: systemPrompt },
      { role: "user", content: jsonElements },
    ],
  });
...
export async function POST(request) {
  try {
    const jsonElements = await request.json();
    const flashcards = await createFlashcards(jsonElements);

    return NextResponse.json({ success: true, flashcards });
  } catch (error) {
    console.error("Error generating flashcards:", error);
    return NextResponse.json(
      { success: false, message: "Failed to generate flashcards." },
      { status: 500 }
    );
  }
}
