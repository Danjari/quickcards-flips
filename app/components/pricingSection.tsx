...
const PricingSection = () => {
    ...
    <section id="pricing" className="h-fit min-h-screen w-full flex flex-col items-center justify-center gap-8 p-8">
        ...
        <div className=' grid grid-cols-1 grid-rows-2 sm:grid-rows-1 md:grid-cols-3 items-center w-full max-w-6xl gap-10'>
            {PricingCards.map((pricingCard) => {
            return (
                <PricingCard oneliner={pricingCard.oneliner} title={pricingCard.title} price={pricingCard.price} features={pricingCard.features} key={pricingCard.id}/>
            )
            })}
            </div>
        </section>
...
interface IPricingCardProps {
  ...
  function PricingCard({title,price,features = [],oneliner}:IPricingCardProps) {
    return (
      <div className='h-fit w-full rounded flex flex-col p-8 gap-8 bg-gray-900 rounded bg-clip-padding backdrop-filter backdrop-blur-xl bg-opacity-20'>
        ...
        <button className='bg-fuchsia-700 rounded p-2 text-sm transition-colors hover:bg-fuchsia-800'>Get Started!</button>
